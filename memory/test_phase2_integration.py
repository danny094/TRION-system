...
def require_db_dir_available():
    if not DB_PATH.parent.exists():
        pytest.skip(f"Memory DB directory not available: {DB_PATH.parent}")
    if not os.access(DB_PATH.parent, os.W_OK):
        pytest.skip(f"Memory DB directory is not writable: {DB_PATH.parent}")
...
def test_3_embedding_pipeline():
    ...
    try:
        ...
        try:
            ...
            if row and row[0] is not None:
                ...
                if emb_row and emb_row[0] == 'task':
                    ...
                    pytest.fail("Embedding not found in embeddings table")
            else:
                print_fail("Embedding ID not set in task_archive")
                pytest.fail("Embedding ID not set in task_archive")
        finally:
            conn.close()
        ...
        if results and len(results) > 0:
            ...
            pytest.fail("Search returned no results")
...
def main():
    ...
    try:
        test_1_flush_mechanism()
        results["Test 1: Flush-Mechanismus"] = True
        test_2_auto_expiry()
        results["Test 2: 48h Auto-Expiry"] = True
        test_3_embedding_pipeline()
        results["Test 3: Embedding-Pipeline"] = True
    finally:

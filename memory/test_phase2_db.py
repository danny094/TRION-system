...
def require_db_available():
    ...
    if not db_dir.exists():
        pytest.skip(f"Memory DB directory not available: {db_dir}")
    if not db_file.exists():
        pytest.skip(f"Memory DB not available: {db_file}")
...
def test_1_schema_verification():
    ...
    try:
        ...
        if task_active_schema:
            ...
            for col in required_cols:
                if col in schema_str:
                    ...
                    pytest.fail(f"Column '{col}' missing in task_active")
        else:
            print_fail("task_active table not found")
            pytest.fail("task_active table not found")
        ...
        if task_archive_schema:
            ...
            for col in required_cols:
                if col in schema_str:
                    ...
                    print_fail(f"  - Column '{col}' missing")
                    pytest.fail(f"Column '{col}' missing in task_archive")
        else:
            print_fail("task_archive table not found")
            pytest.fail("task_archive table not found")
        ...
        if index_exists:
            ...
            pytest.fail("Index idx_task_active_conv missing")
...
def test_2_basic_operations():
    ...
    try:
        ...
        if count == 15:
            ...
            conn.close()
            pytest.fail(f"Expected 15 tasks, got {count}")
        ...
        if active_count == 10:
            ...
            conn.close()
            pytest.fail(f"Expected 10 active tasks, got {active_count}")
        ...
        if archive_count == 5:
            ...
            conn.close()
            pytest.fail(f"Expected 5 archived tasks, got {archive_count}")
...
def test_3_expiry_simulation():
    ...
    try:
        ...
        if active_count == 2:
            ...
            conn.close()
            pytest.fail(f"Expected 2 active tasks, got {active_count}")
        ...
        if archive_count == 3:
            ...
            conn.close()
            pytest.fail(f"Expected 3 archived tasks, got {archive_count}")
...
def main():
    ...
    try:
        test_1_schema_verification()
        results["Schema Verification"] = True
        test_2_basic_operations()
        results["Basic CRUD Operations"] = True
        test_3_expiry_simulation()
        results["48h Auto-Expiry"] = True
    finally:

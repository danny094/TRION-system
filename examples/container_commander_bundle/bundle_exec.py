...
from bundle_common import error_result, is_not_found, resolve_container_reference
...
def _allowed_exec(blueprint_id):
    detail = bundle_blueprint_store.get_blueprint(blueprint_id)
...
def container_exec(container_id="", command="", timeout=30, container_name=""):
...
def container_exec_detailed(container_id="", command="", timeout=30, container_name=""):
    container_ref = container_id
    try:
        client = bundle_docker.get_docker_client()
        container = resolve_container_reference(client, container_ref)

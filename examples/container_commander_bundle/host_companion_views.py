...
LOCAL_PACKAGE_DIR = BUNDLE_ROOT / "marketplace" / "packages"


def _load_local_package_manifest(blueprint_id):
...
def check_host_companion(blueprint_id):
...
def repair_host_companion(blueprint_id):
    manifest, host_companion = _host_companion_config(blueprint_id)
...
def uninstall_host_companion(blueprint_id):
    manifest, host_companion = _host_companion_config(blueprint_id)
    if not host_companion:
        return {
            "uninstalled": False,

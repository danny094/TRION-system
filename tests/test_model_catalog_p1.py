...
def test_models_catalog_returns_provider_aware_selected_model(monkeypatch):
    ...
    async def fake_fetch_cloud_models(endpoint, headers=None):
    ...
    async def fake_resolve_key(provider):
        ...
        # Cloud-Provider auf (ollama_cloud, openrouter, deepseek, minimax), nicht mehr
        # nur fuer ollama_cloud. Fake muss alle vier beantworten koennen.
        assert provider in {"ollama_cloud", "openrouter", "deepseek", "minimax"}
        return "cloud-key" if provider in {"ollama_cloud", "deepseek"} else ""

    monkeypatch.setattr(models_routes, "_fetch_tags", fake_fetch_tags)
    monkeypatch.setattr(models_routes, "_fetch_openai_compatible_models", fake_fetch_cloud_models)
    ...
    # W1/SP4.1: provider_ids() liefert inzwischen auch Cloud-Provider
    # (core/llm/provider_registry.py PROVIDER_SPECS), Test stammte aus der
    # Zeit vor deren Einfuehrung.
    assert payload["providers"] == ["ollama", "ollama_cloud", "openai", "anthropic", "openrouter", "deepseek", "minimax"]
    ...
    assert {"name": "deepseek-v4-pro", "provider": "deepseek", "source": "cloud", "selected": False} in payload["models"]
    assert any(row["name"] == "deepseek-v4-flash" and row["provider"] == "deepseek" for row in payload["models"])

...
from core.llm.provider_registry import openrouter_base
...
async def _headers(provider: str) -> Dict[str, str]:
    ...
    title = str(os.getenv("OPENROUTER_APP_NAME", "")).strip()
    if referer:
...
async def complete_chat(
    *,
    ...
    async with httpx.AsyncClient(timeout=timeout_s) as client:
        response = await client.post(f"{openrouter_base()}/chat/completions", json=body, headers=await _headers(provider))
...
async def stream_chat_events(
    *,
    ...
    async with httpx.AsyncClient(timeout=timeout_s) as client:
        async with client.stream("POST", f"{openrouter_base()}/chat/completions", json=body, headers=await _headers(provider)) as response:
            capture_rate_limit_headers(provider, response.headers, response.status_code)
            response.raise_for_status()

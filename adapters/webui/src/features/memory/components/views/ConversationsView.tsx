...
export function ConversationsView() {
  ...
  async function reloadDetail(conversationId: string) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t('memory.detailLoadFailed'))
    } finally {
    ...
    return (
      <div className="px-8 py-7">
        ...
        >
          ...
          {t('memory.backToConversations')}
        </button>
        <div className="mt-3 flex items-end justify-between gap-4">
          <div>
            <div className="text-[10px] uppercase tracking-[0.22em] text-white/40">{t('memory.conversation')}</div>
        ...
        <div className="mt-6 space-y-2.5">
          {loadingDetail ? (
            <div className="rounded-2xl border border-white/5 bg-white/[0.02] px-4 py-8 text-center text-[12px] text-white/45">{t('memory.loading')}</div>
          ) : entries.length === 0 ? (
            <div className="rounded-2xl border border-white/5 bg-white/[0.02] px-4 py-8 text-center text-[12px] text-white/45">{t('memory.noEntries')}</div>
          ) : (
            entries.map((entry) => (
              <MemoryEntryItem
                key={entry.id}
    ...
    <div className="px-8 py-7">
      <div className="flex items-end justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 text-[22px] font-semibold text-white/92">{t('memory.conversationsTitle')}</h1>
          <p className="mt-1 text-[12px] text-white/55">{t('memory.conversationsDescription')}</p>
        </div>
        <button
          type="button"
          onClick={reloadList}
      ...
      <div className="mt-6 space-y-1.5">
        {loadingList && conversations.length === 0 ? (
          ...
          <div className="rounded-2xl border border-white/5 bg-white/[0.02] px-4 py-8 text-center text-[12px] text-white/45">{t('memory.noneConversations')}</div>
        ) : (
              ...
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1 flex gap-3 text-[10px] uppercase tracking-[0.16em] text-white/40">
                    {typeof conv.entry_count === 'number' ? <span>{t('memory.entries', { count: conv.entry_count })}</span> : null}
                    {conv.last_activity_at ? <span>{formatTime(conv.last_activity_at, locale)}</span> : null}
                  </div>
                </div>

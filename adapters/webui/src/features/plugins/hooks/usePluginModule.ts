  ...
  useEffect(() => {
    ...
    import(/* @vite-ignore */ pluginAssetUrl(plugin, plugin.entry))
      .then((mod) => {
        ...
        setState(Component ? { Component, error: null } : { Component: null, error: 'Plugin exportiert keine React-Komponente als default.' })
      })
      .catch((error: unknown) => {
        if (cancelled) return
        setState({ Component: null, error: error instanceof Error ? error.message : 'Plugin-Modul konnte nicht geladen werden.' })
      })
    return () => {
      cancelled = true
    }

    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                return {"status": "ready", "service": "online"}
    except Exception as e:
        return {"status": "error", "service": "offline", "error": str(e)}
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                ...
                json={
                    ...
                    "id": 1,
                    "method": "tools/call",
                    "params": {"name": "maintenance_run", "arguments": {}},
                },
                timeout=60.0,
            )
            if response.status_code == 200:
                return {"status": "success", "message": "Maintenance started"}
            raise HTTPException(status_code=response.status_code, detail=f"Memory service returned {response.status_code}")

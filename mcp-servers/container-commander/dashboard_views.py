...
from network_views import list_networks
from proxy_views import get_whitelist
from runtime_views import list_containers
from volume_views import list_volumes
...
def _runtime_error(result: dict[str, Any]) -> str:
...
def get_dashboard_overview() -> dict[str, Any]:
    ...
    if blueprints_result.get("ok") is False:
        alert_messages.append(
            f"blueprint_store_unavailable:{str((blueprints_result.get('error') or {}).get('message') or 'unknown')}"
        )

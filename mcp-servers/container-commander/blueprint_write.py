...
def create_blueprint(definition: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        conn.close()
    detail = get_blueprint(params["id"])
    return {"created": True, "blueprint": detail.get("blueprint", {}), "trust": _trust(detail.get("blueprint", {}))}
...
def update_blueprint(blueprint_id: str, updates: dict[str, Any]) -> dict[str, Any]:
    ...
    params = _normalize(updates, existing=existing)
    conn = get_conn()
    try:
        conn.execute(
            """
    ...
    return {"updated": True, "blueprint": detail.get("blueprint", {}), "trust": _trust(detail.get("blueprint", {}))}
